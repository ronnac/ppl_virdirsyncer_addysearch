...
choose_entry() {
    ...
    if [ "$CliOnly" == "true" ];then
        SelectedVcard=$(rg "FN:" ~/.contacts/contacts/* | awk -F ':' '{print $3 ":" $1 }' | fzf --no-hscroll -m --height 50% --border --ansi --no-bold --header "Whose Vcard?" --preview="$SCRIPTDIR/vcardreader.sh {}"  | awk -F ':' '{print $2}' )
    else
        SelectedVcard=$(rg "FN:" ~/.contacts/contacts/* | awk -F ':' '{print $3 ":" $1 }' | rofi -i -dmenu -p "Whose Vcard?" | awk -F ':' '{print $2}' )
    fi
